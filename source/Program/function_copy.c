	...
	while ((entry=function_get_entry(handle)))
	{
...
#ifdef __amigaos4__
				if (!(icon=GetIconTags(handle->work_buffer,
...
#ifdef __amigaos4__
					icon=GetIconTags(NULL,ICONGETA_GetDefaultType,WBDRAWER,TAG_DONE);
#else
				...
				if ((path=function_path_current(&handle->dest_paths)))
				{
					...
					if ((lock=OriginalCreateDir(handle->work_buffer)))
					{
						...
						if (icon)
						{
							// Write icon
							#ifdef __amigaos4__
							if (PutIconTags(handle->work_buffer,icon,TAG_DONE))
#else
		...
		if (entry->flags&FUNCENTF_TOP_LEVEL)
		{
			...
			else if (entry->type > 0)
			{
				...
				{
#ifdef __amigaos4__
					if (ExamineObject_to_FIB(lock, fib))
#else
					if (Examine(lock, fib))
#endif
					{
		...
		if (file_ok)
		{
			...
			while (path)
			{
				...
				if (dest_count>1 || change_info)
				{
					...
					// Clear filename
					if ((file=(STRPTR)MutableFilePart(source_file)))
					{
						ch=*file;
						*file=0;
					}
							...
							while (1)
							{
								...
								if (!error)
								{
									...
									if ((dest=Lock(dest_file,ACCESS_READ)))
									{
										...
										// Use new filename
										strcpy(handle->d_info->fib_FileName,MutableFilePart(dest_file));
										ret=1;
									}
									break;
								}
	...
	*err_code=0;
		...
		(dest_ptr=isicon(dest_file)))
	{
		...
		*dest_ptr=0;
		...
		// Try to get icon
#ifdef __amigaos4__
		if ((icon=GetIconTags(source_file,
			ICONGETA_FailIfUnavailable,TRUE,
			TAG_DONE)))
#else
...
#endif
		{
			...
			if (copy_flags&COPY_TOP_LEVEL)
				copy_icon_position(handle,(STRPTR)MutableFilePart(source_file),icon);
	...
	#ifdef __amigaos4__
		if (PutIconTags(dest_file,icon,TAG_DONE))
#else
