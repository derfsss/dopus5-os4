...
void clock_show_memory(struct RastPort *rp,long msg,long clock_x,char *error)
{
	...
	{
#ifdef __amigaos4__
		// On OS4, chip/fast split is meaningless — show total free as smart KB/MB
		{
			...
			lsprintf(GUI->screen_title,"%s  %s free",dopus_name,membuf);
		}
#else
		// Thousands separator?
...
//#endif
		...
		BltBitMapTags(
			...
			BLITA_DestX,x,
			BLITA_DestY,(rp->TxHeight+2-size)>>1,
			BLITA_Width,size,
			BLITA_Height,size,
			TAG_END);
#else
