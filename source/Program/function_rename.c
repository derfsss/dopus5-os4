	...
	while ((entry=function_get_entry(handle)))
	{
		...
		else
		{
			...
			if (!data->wild_flag)
			{
				...
				{
					// Store names
					strcpy(data->old_name_edit,MutableFilePart((char *)instruction->funcargs->FA_Arguments[RENAME_NAME]));
					strcpy(data->new_name_edit,MutableFilePart((char *)instruction->funcargs->FA_Arguments[RENAME_NEWNAME]));
					ret=1;
				}
				...
				if (ret==1)
				{
					...
					if ((lock=Lock(dest_file,ACCESS_READ)))
					{
						...
						if (entry->entry && entry->entry->de_Flags&ENTF_LINK)
						{
							...
							strcpy(handle->s_info->fib_FileName,MutableFilePart(dest_file));
