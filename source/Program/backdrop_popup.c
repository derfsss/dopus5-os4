			...
			if (flags&BPF_FTYPE && item && item->data)
			{
				...
				if (lock || devptr)
				{
					...
					if ((array=NewArgArray()))
					{
						...
						// Get entry
						if ((entry=NewArgArrayEntry(array,(devptr)?devptr:((filename)?(char *)MutableFilePart(filename):object->name))))
						{
							// Directory?
...
void popup_build_copyto(PopUpHandle *menu,PopUpItem *item)
{
	...
	if ((lock=Lock("dopus5:system/CopyTo",ACCESS_READ)))
	{
		...
		if (fib->fib_DirEntryType>0)
		{
			...
			// Scan directory
#ifdef __amigaos4__
			context = ObtainDirContextTags(EX_LockInput, lock, TAG_END);
			while (context && (exdata = ExamineDir(context)))
			{
...
#ifdef __amigaos4__
			if (context) ReleaseDirContext(context);
#endif
		}
...
void popup_build_openwith(PopUpHandle *menu)
{
	...
	else
	{
			...
			(PopUpItemSub(menu,item)))
		{
			...
			{
				// Add item
				PopUpNewItem(menu,(ULONG)MutableFilePart(node->ln_Name),MENU_OPEN_WITH_BASE+count,POPUPF_STRING);
			}
