...
void close_display(int close_flags,BOOL quit_all)
{
	...
	if (close_flags&(CLOSE_SCREEN|CLOSE_CHILDREN))
	{
		...
		// Tell all processes to hide (async — cannot be synchronous because
		// environment_proc is on process_list and may be the caller, causing deadlock)
	...
	if (close_flags&CLOSE_SCREEN)
	{
			...
			!(CloseScreen(GUI->screen)))
		{
			...
			while (GUI->screen)
			{
				...
				// Put up requester (Try Again=1, Abort=0)
