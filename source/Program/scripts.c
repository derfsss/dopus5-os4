...
void InitSoundEvents(BOOL force)
{
	...
	for (sound=(Cfg_SoundEntry *)environment->sound_list.mlh_Head;sound->dse_Node.ln_Succ;sound=(Cfg_SoundEntry *)sound->dse_Node.ln_Succ)
	{
		...
		if (sound->dse_Flags&CFGSEF_CHANGED || force)
		{
			...
			if ((ptr=MutableFilePart(sound->dse_Sound)) && (strchr(ptr,'*') || strchr(ptr,'?') || strchr(ptr,'#')))
				wild=1;
			...
			while (!node)
			{
				...
				if (stricmp(node->node.ln_Name,MutableFilePart(buf+200))!=0 || count==1)
					break;
				node=0;
			}
			...
			if (node)
			{
				...
				if (wild && (ptr=MutableFilePart(buf+200))) *ptr=0;
