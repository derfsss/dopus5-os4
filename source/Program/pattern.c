...
BOOL pattern_check_random(PatternData *data,char *name)
{
	...
	if ((ptr=MutableFilePart(data->data)) && (strchr(ptr,'*') || strchr(ptr,'?') || strchr(ptr,'#')))
		wild=1;
	...
	while (!node)
	{
		...
		if (stricmp(node->node.ln_Name,MutableFilePart(buf+64))!=0 || count==1)
			break;
		node=0;
	}
	...
	if (node)
	{
		...
		if (wild && (ptr=MutableFilePart(buf+64))) *ptr=0;
