	...
	ReselectionData *reselect)
{
	...
	if (!ExamineObject_to_FIB(lock,fileinfo))
	...
	else
	{
		...
		else stccpy(buffer->buf_ObjectName,MutableFilePart(buffer->buf_ExpandedPath),GUI->def_filename_length-1);
	}
...
#if defined(__amigaos4__)
	{
		...
		if ((context = ObtainDirContextTags(EX_LockInput, lock,
				EX_DataFields, EXF_ALL, TAG_END)))
		{
			while ((exdata = ExamineDir(context)))
			{
				...
				// Fill legacy FIB from ExamineData
				ExamineData_to_FIB(exdata, fileinfo);
				((FileInfoBlock64 *)fileinfo)->fib_Size64 = exdata->FileSize;
				...
				if (ok)
				{
					...
					// Create entry — use exdata->Name directly to support names >107 chars
