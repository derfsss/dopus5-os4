...
Cfg_Environment *environment_new(void)
{
	...
	if (!(env->memory=NewMemHandle(0,0,MEMF_CLEAR)))
		{ environment_free(env); return 0; }

	if (!(env->desktop_memory=NewMemHandle(1024,512,MEMF_PUBLIC|MEMF_CLEAR)))
		{ environment_free(env); return 0; }

	if (!(env->volatile_memory=NewMemHandle(1024,sizeof(ButtonBankNode)+256,MEMF_PUBLIC|MEMF_CLEAR)))
		{ environment_free(env); return 0; }
...
BOOL environment_open(Cfg_Environment *env,char *name,BOOL first,APTR prog)
{
	...
	if ((success=OpenEnvironment((name)?name:env->path,opendata)))
	{
		...
		if (GUI->def_filename_length>MAX_FILENAME_LEN)
