	...
	short flags)
{
	...
	if (flags&FFLF_RELOAD)
	{
		...
		change=function_filechange_delfile(handle,path,MutableFilePart(buffer),0,0);
	}
					...
					if (change->node.ln_Type==FCTYPE_RENAME)
					{
						...
						if (strreplace(buffer->buf_Path,list->path,change->fib->fib_FileName,1))
						{
							...
							if (stricmp(buffer->buf_Path,change->fib->fib_FileName)==0)
							{
								...
								// Copy object name
								strncpy(buffer->buf_ObjectName,MutableFilePart(change->fib->fib_FileName),GUI->def_filename_length-1);
								len=-1;
