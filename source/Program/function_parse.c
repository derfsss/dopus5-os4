	...
	for (pos=0;pos<func_len && func_pos<limit;pos++)
	{
		...
		switch (ch)
		{
			...
			case FUNC_LAST_FILE:
				...
				func_pos+=function_add_filename(
					function_buf,
					(ch==FUNC_LAST_PATH)?(char *)handle->last_filename:(char *)MutableFilePart(handle->last_filename),
					0,
					1,
					(wb_func && entry->type>ENTRY_DEVICE));
				break;
	...
	for (str_pos=0,buf_pos=0;str_pos<len && buf_pos<511;str_pos++)
	{
		...
		if (def_string[str_pos]=='[' && def_string[str_pos+2]==']')
		{
			...
			switch (def_string[str_pos+1])
			{
				...
				// Filename
				case 'o':
					if ((ptr=MutableFilePart(handle->last_filename)))
	...
	else
	{
		...
		*temp_buffer=0;
		...
		if (type=='f' || type=='F' || type=='d')
		{
			...
			// Get file pointer
			if ((file=MutableFilePart(temp_buffer)))
			{
				// Copy file
