	...
	while ((entry=function_get_entry(handle)))
	{
		...
		else
		{
			...
			if (entry->type<0)
			{
				...
				{
					// Match name
					match=MatchPatternNoCase(handle->inst_data+64,MutableFilePart(entry->name));
				}
				...
				if (match)
				{
					...
					// Strip filename
					if ((ptr=MutableFilePart(handle->work_buffer+512))) *ptr=0;
					get_trunc_filename(MutableFilePart(entry->name),name);
					...
					if (ret==1)
					{
							...
							path->lister)
						{
							...
							function_select_file(handle,path->lister,(data->comment)?(char *)MutableFilePart(entry->name):data->pattern);
					...
					if (ret==2)
					{
								...
								function_lister_current(&handle->source_paths))))
						{
							...
							// Do wildcard selection
							function_select_file(handle,lister,(data->comment)?(char *)MutableFilePart(entry->name):data->pattern);
						}
					}
				}
			}
