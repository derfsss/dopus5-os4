...
DOPUS_FUNC(function_runprog)
{
	...
	while ((entry=function_get_entry(handle)))
	{
		...
		#ifdef __amigaos4__
		if (!(handle->flags&FUNCF_RUN_NO_ICONS) && (icon=GetIconTags(entry->name,
...
#endif
		{
				...
				icon->do_DefaultTool[0])
			{
				...
				else tool=MutableFilePart(icon->do_DefaultTool);
			}

			// Get pointer to file name
			else tool=MutableFilePart(entry->name);
		...
		if ((file=Open(entry->name,MODE_OLDFILE)))
		{
			...
			*args=0;
			...
			if ((file=Lock(entry->name,ACCESS_READ)))
			{
				// Examine file
#ifdef __amigaos4__
				ExamineObject_to_FIB(file,handle->s_info);
#else
...
#ifdef __amigaos4__
	// Defensive: verify lock is still valid before unlocking.
	...
	// NameFromLock() will fail safely if the lock is invalid.
	{
		char test_buf[4];
