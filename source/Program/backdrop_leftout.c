	...
	ULONG flags)
{
	...
	if (!parent_dir)
	{
		...
		if (!(lock=Lock(name,ACCESS_READ)))
		{
			...
			if (stricmp(MutableFilePart(name),"disk")==0)
				return 0;
	...
	if ((newob=AllocMemH(info->memory,size)))
	{
		...
		strcpy(newob->name,MutableFilePart(name));
	...
	short x,short y)
{
	...
	if (!(lock=Lock(name,ACCESS_READ)))
	{
		...
		// Isolate filename
		stccpy(fib->fib_FileName,MutableFilePart(name),sizeof(fib->fib_FileName));
	}

	// Locked object, get some info
#ifdef __amigaos4__
	else ExamineObject_to_FIB(lock,fib);
#else
	...
	if (!object)
	{
		...
		{
			// Set name
			stccpy(object->device_name,MutableFilePart(name),GUI->def_filename_length);
			...
			!(object->flags&(BDOF_TEMP_LEFTOUT|BDOF_DESKTOP_FOLDER)))
		{
			...
			if (!entry)
			{
				...
				if ((left=AllocMemH(GUI->position_memory,sizeof(leftout_record)+strlen(info->buffer))))
				{
					...
					stccpy(left->icon_label,MutableFilePart(info->buffer),sizeof(left->icon_label));
...
void backdrop_add_leftouts(BackdropInfo *info)
{
		...
		left=(leftout_record *)left->node.ln_Succ)
	{
		...
		if (left->node.ln_Type==PTYPE_LEFTOUT)
		{
			...
			// Get filename
			name=MutableFilePart(left->node.ln_Name);
				...
				if ((backdrop=Open(buf,MODE_OLDFILE)))
				{
					while (FGets(backdrop,leftout,sizeof(leftout)-1) )
					{
						...
						if (*leftout)
						{
							...
							{
								stccpy(newobject->device_name,MutableFilePart(buf),GUI->def_filename_length+1);
	...
	short x,short y)
{
	...
	if (MakeLink(info->buffer,name,TRUE))
	{
		...
		if ((icon=GetProperIcon(name,0,GCDOF_NOCACHE)))
		{
			...
			#ifdef __amigaos4__
			PutIconTags(info->buffer,icon,TAG_DONE);
#else
...
void backdrop_unique_name(char *name)
{
	...
	for (try=0;try<100;try++)
	{
		...
		if (!ptr)
		{
			...
			if (strlen(MutableFilePart(name))>26)
			{
