...
#ifdef __amigaos4__
	PutIconTags(dest,icon,TAG_DONE);
#else
	...
	REG(d0, BOOL save))
{
	...
	*MutablePathPart(path) = '\0';
	...
	if ((SetVar(name,data,-1,GVF_GLOBAL_ONLY)) && save)
	{
		...
		// Build full ENVARC: path
		if ((fullpath = AllocVec(strlen("envarc:") + strlen(name) + 1, MEMF_ANY)))
		{
			char *dirpart;
			BPTR dirlock;

			strcpy(fullpath, "envarc:");
			strcat(fullpath, name);

			// Create directory tree for the path
			dirpart = MutablePathPart(fullpath);
			if (dirpart > fullpath)
			{
				char saved = *dirpart;
				*dirpart = 0;
				if ((dirlock = CreateDirTree(fullpath)))
					UnLock(dirlock);
				*dirpart = saved;
			}

			// Write the file
			if ((file = Open(fullpath, MODE_NEWFILE)))
			{
