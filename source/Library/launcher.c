	...
	struct MsgPort *reply)
{
	...
	if ((icon=GetIconTags(name,
		...
		TAG_DONE)) && icon->do_Type==WBPROJECT)
#else
...
#else
		run_name=MutableFilePart(default_tool);
#endif
	...
	launch_setarg(launch,arg++,parent,MutableFilePart(name));
	...
	for (;arg<launch->startup.sm_NumArgs;arg++)
	{
		...
		// Fill out argument
		launch_setarg(launch,arg,parent,MutableFilePart(buf));
	}
	...
	// Get process name
	proc_name=MutableFilePart(run_name);
