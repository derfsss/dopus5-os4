	...
	REG(d0, ULONG size))
{
...
#ifdef __amigaos4__
	if (!handle) mem=AllocVecTags(size,AVT_ClearWithValue,0,TAG_DONE);
#else
	...
	else
	{
		...
		else
		{
...
#ifdef __amigaos4__
			/* OS4 AllocVecTags needs a valid memory type; 0 is not valid.
			   Classic AllocMem(size,0) means "any memory" but OS4 needs MEMF_PRIVATE. */
			if ((mem=AllocVecTags(size,
				AVT_Type,(handle->type&~MEMF_CLEAR)?handle->type&~MEMF_CLEAR:MEMF_PRIVATE,
				(handle->flags&MEMHF_CLEAR)?AVT_ClearWithValue:TAG_IGNORE,0,
				TAG_DONE)))
#else
			...
			/* else: allocation failed, mem remains NULL */
		}
