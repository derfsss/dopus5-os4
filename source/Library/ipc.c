	...
	REG(a6, struct MyLibrary *libbase))
{
	...
	if (!(ipc=AllocVec(sizeof(IPCData),MEMF_CLEAR)) ||
		!((tags=AllocVec(sizeof(struct TagItem)*9,MEMF_ANY))))
	{
		FreeVec(ipc);
		return 0;
	}
	...
	// Mark as child process on OS4
	#ifdef __amigaos4__
	tags[5].ti_Tag=NP_Child;
	tags[5].ti_Data=TRUE;
	#define IPC_TAG_BASE 6
	#else
	#define IPC_TAG_BASE 5
	#endif
...
#undef DOpusBase
		...
		// Fill out tags
		tags[IPC_TAG_BASE].ti_Tag=NP_Cli;
		tags[IPC_TAG_BASE].ti_Data=TRUE;
		tags[IPC_TAG_BASE+1].ti_Tag=NP_Path;
		tags[IPC_TAG_BASE+1].ti_Data=(ULONG)pathlist;
		tags[IPC_TAG_BASE+2].ti_Tag=TAG_END;
	}
	else tags[IPC_TAG_BASE].ti_Tag=TAG_END;
	#undef IPC_TAG_BASE
