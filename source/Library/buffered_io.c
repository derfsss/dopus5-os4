...
#ifdef __amigaos4__
		if (ChangeFilePosition(((BufFile *)file)->file,offset,mode))
		{
			new_pos=GetFilePosition(((BufFile *)file)->file);
			((BufFile *)file)->file_pos=new_pos;
		}
		else new_pos=-1;
#else
...
{
	#ifdef __amigaos4__
	return ExamineObject_to_FIB_FH(((BufFile *)file)->file,fib);
	#else
