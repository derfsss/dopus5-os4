...
BOOL play_file(play_data *data)
{
	...
	play_update_info(data,MutableFilePart(data->file),0,0,GetString(locale,MSG_STATUS_LOADING));
...
void play_iconify(play_data *data)
{
	...
	if (!data->icon &&
#ifdef __amigaos4__
		!(data->icon=GetIconTags("dopus5:icons/play",
...
#endif
	#ifdef __amigaos4__
		!(data->icon=GetIconTags(NULL,ICONGETA_GetDefaultType,WBPROJECT,TAG_DONE))) return;
#else
