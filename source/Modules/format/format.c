	...
	if ((lock=Lock(disk->dh_name,ACCESS_READ)))
	{
		...
		switch (data->info.id_DiskType)
		{
			...
			default:
				...
				// Get disk name
				#ifdef __amigaos4__
				ExamineObject_to_FIB(lock,&data->fib);
				#else
				...
				{
					APTR context;
					if ((context = ObtainDirContextTags(EX_LockInput, lock, EX_DoCurrentDir, TRUE, TAG_END)))
					{
						if (ExamineDir(context))
							blank=0;
...
BOOL do_format(format_data *data,DiskHandle *disk,unsigned short type,short *noactive)
{
	...
	*noactive=0;
	...
	if (suc && data->default_trash)
	{
		...
		// Create directory
#ifdef __amigaos4__
		if ((lock=CreateDirTree(trash_name)))
#else
...
#endif
		{
			...
			// Add icon
#ifdef __amigaos4__
			if ((trash=GetIconTags(NULL,ICONGETA_GetDefaultType,WBGARBAGE,TAG_DONE)))
#else
			if ((trash=GetDefDiskObject(WBGARBAGE)))
#endif
			{
	#ifdef __amigaos4__
				PutIconTags(trash_name,trash,TAG_DONE);
#else
