	...
	REG(d1, ULONG mod_data))
{
	...
	if (read_open_window(data))
	{
		...
		for (node=files->lh_Head;node->ln_Succ;node=node->ln_Succ)
		{
			...
			// Show title
			lsprintf(data->title,GetString(locale,MSG_READING_FILE),(IPTR)MutableFilePart(data->file));
...
BOOL read_file(read_data *data,ULONG type)
{
	...
	if (!(file=Lock(data->file,ACCESS_READ)) ||
		#ifdef __amigaos4__
		!(ExamineObject_to_FIB(file,fib))) ret=0;
		#else
				...
				switch (msg_copy.Class)
				{
					...
					case IDCMP_GADGETUP:
							...
							data->appport)
						{
							...
							if (!data->app_diskobj &&
#ifdef __amigaos4__
								!(data->app_diskobj=GetIconTags("dopus5:icons/read",
...
#endif
	#ifdef __amigaos4__
								!(data->app_diskobj=GetIconTags(NULL,ICONGETA_GetDefaultType,WBPROJECT,TAG_DONE))) break;
#else
...
void read_build_title(read_data *data)
{
	...
	if (!(IsListEmpty((struct List *)&data->text_data)))
	{
		...
		lsprintf(data->title,GetString(locale,MSG_FILE_TITLE),
			(IPTR)MutableFilePart(data->file),
			(IPTR)data->date,
			data->lines,
			data->size,
...
void read_save(read_data *data)
{
	...
	tags[2].ti_Data=(ULONG)MutableFilePart(data->file);
	tags[3].ti_Tag=TAG_DONE;

	// Get initial drawer
	strcpy(data->line_buffer,data->file);
	if ((ptr=MutableFilePart(data->line_buffer))) *ptr=0;
