...
#ifdef __amigaos4__
				BltBitMapTags(
					BLITA_Source,data->dt_bm,
					BLITA_SrcType,BLITT_BITMAP,
					BLITA_SrcX,0,
					BLITA_SrcY,0,
					BLITA_Dest,data->display_window->RPort,
					BLITA_DestType,BLITT_RASTPORT,
					BLITA_DestX,0,
					BLITA_DestY,0,
					BLITA_Width,GetBitMapAttr(data->dt_bm,BMA_WIDTH),
					BLITA_Height,GetBitMapAttr(data->dt_bm,BMA_HEIGHT),
					TAG_END);
#else
				BltBitMapRastPort(
					data->dt_bm,0,0,
...
BOOL show_help(show_data *data)
{
	...
	if ((help_window=OpenConfigWindow(&new_win)))
	{
		...
		// File name
		SetGadgetValue(list,GAD_INFO_FILE,(ULONG)MutableFilePart(data->file));
