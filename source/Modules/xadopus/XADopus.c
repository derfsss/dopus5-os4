...
void BuildTree(struct xoData *data)
{
	...
	while(xfi) {
		...
		name=MutableFilePart(xfi->xfi_FileName);
		dup=FALSE;
	...
	while(tmp && (!over)) {
		...
		if(tmp) {
			...
			if(!skip) {
				...
				sprintf(data->buf,"lister select %s \"%s\" off",data->lists,MutableFilePart(tmp->fib.fib_FileName));
				DC_CALL4(infoptr, dc_SendCommand,
					DC_REGA0, IPCDATA(data->ipc),
					DC_REGA1, data->buf,
					DC_REGA2, NULL,
	...
	if((list=Att_NewList(0))) {
		if((dl=LockDosList(LDF_DEVICES|LDF_READ))) {
			...
			if((sel>=0) && (dev=xadAllocObject(XADOBJ_DEVICEINFO,(IPTR)NULL))) {
				SetProgressWindowTags(data->ptr,
						PW_FileName,	(IPTR)MutableFilePart(data->arcname),
						PW_Info,	    (IPTR)"",
						PW_FileDone,	0,
...
BOOL ExtractF(struct xoData *data)
{
	...
	while(xfi && (!over)) {
		...
		*((char *)MutableFilePart(FileName))=0;

		SetProgressWindowTags(data->ptr,
			PW_FileName,	(IPTR)MutableFilePart(xfi->xfi_FileName),
			PW_FileNum,	    ++total,
...
//	if( (!(data.listp2 = data.hook.dc_GetSource(IPCDATA(ipc), arcname))) || (!(Entry=data.hook.dc_GetEntry(IPCDATA(ipc)))) )
	...
	if(mod_id == 1)
	{
		...
		if((data.ArcInf=xadAllocObject(XADOBJ_ARCHIVEINFO,(IPTR)NULL))) {
			...
			while(Entry && (!over)) {
				SetProgressWindowTags(data.ptr,
					PW_Info,      (IPTR)MutableFilePart(arcname),
				...
				if (Entry) {
					*((char *)MutablePathPart(arcname))=0;
			...
			*data.listp.buffer = data.listp.flags = 0;
			...
			if((data.ArcInf = xadAllocObject(XADOBJ_ARCHIVEINFO, (IPTR)NULL)))
			{
				...
				if(!err)
				{
					...
					sprintf(buf, "lister set %s title XADopus: %s", data.lists, MutableFilePart(arcname));
					DC_CALL4(infoptr, dc_SendCommand,
						DC_REGA0, IPCDATA(ipc),
						DC_REGA1, buf,
						DC_REGA2, NULL,
