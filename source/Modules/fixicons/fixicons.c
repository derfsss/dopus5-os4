...
if	((l->obj = GetDiskObject( path )))
	...
	#ifdef __amigaos4__
		if	((r = PutIconTags( path, l->obj, TAG_DONE )))
#else
...
if	(!l->isicon && (l->lock = Lock( path, ACCESS_READ )))
	...
	exdata = ExamineObjectTags(EX_LockInput, l->lock, TAG_END);
	if (exdata && EXD_IS_DIRECTORY(exdata))
		{
		APTR context;
		FreeDosObject(DOS_EXAMINEDATA, exdata);

		if ((context = ObtainDirContextTags(EX_LockInput, l->lock, TAG_END)))
			{
			while ((exdata = ExamineDir(context)))
				{
				ExamineData_to_FIB(exdata, &l->fib);
				...
				SetProgressWindowTags(progress,PW_FileName,(IPTR)MutableFilePart(path),TAG_END);
...
if	(!args || !args->FA_Arguments[FI_NFO])
	...
	for	(node=(Att_Node *)files->list.lh_Head;node->node.ln_Succ;node=(Att_Node *)node->node.ln_Succ)
		{
		if	(!stricmp( "Disk.info", MutableFilePart( node->node.ln_Name )))
			{
			// Not a full path?
	...
	#ifdef __amigaos4__
						if	((result = PutIconTags( path, obj, TAG_DONE )))
#else
...
for	(node=(Att_Node *)files->list.lh_Head;node->node.ln_Succ;node=(Att_Node *)node->node.ln_Succ,count++)
	...
	SetProgressWindowTags(
		progress,
		PW_FileName,(IPTR)MutableFilePart(path),
		PW_FileNum,count,
		TAG_END);
