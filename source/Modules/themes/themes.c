	...
	REG(d1, EXT_FUNC(func_callback)))
{
	...
	if (mod_id==SAVETHEME || mod_id==BUILDTHEME)
	{
				...
				(1<<30)|(1<<31)|FRF_DOSAVEMODE,0))*/
		{
			...
			// Remove spaces
			for (ptr=MutableFilePart(filename);*ptr;ptr++)
				if (*ptr==' ') *ptr='_';
	...
	if (mod_id==CONVERTTHEME)
	{
				...
				"#?.theme"))*/
		{
					...
					(1<<30)|(1<<31)|FRF_DOSAVEMODE,0)))*/
			{
				...
				// Remove spaces
				for (ptr=MutableFilePart(dest);*ptr;ptr++)
					if (*ptr==' ') *ptr='_';
...
#ifdef __amigaos4__
		if ((lock=CreateDirTree(build_path)))
			UnLock(lock);
...
BOOL save_theme_background(APTR file,DOpusCallbackInfo *info,char *type,struct MsgPort *reply_port,char *build_path,APTR progress)
{
	...
	if ((len=strlen(temp))>0)
	{
		...
		if (*build_path && temp[len-1]!='/' && !strchr(temp,'*') && !strchr(temp,'#') && !strchr(temp,'?'))
		{
			...
			// Add filename
			AddPart(dest,MutableFilePart(temp),340);

			// Try to copy file
			if (theme_copy_file(temp,dest))
				lsprintf(temp,"D5THEMES:%s/Screens/%s",(IPTR)MutableFilePart(build_path),(IPTR)MutableFilePart(dest));
			else
				ret=0;
		}
...
BOOL save_theme_sound(APTR file,DOpusCallbackInfo *info,char *type,struct MsgPort *reply_port,char *build_path,APTR progress)
{
	...
	if ((len=strlen(temp))>0)
	{
		...
		if (*build_path && temp[len-1]!='/' && !strchr(temp,'*') && !strchr(temp,'#') && !strchr(temp,'?'))
		{
			...
			// Try to copy file
			if (theme_copy_file(temp,dest))
				lsprintf(temp,"D5THEMES:%s/Sounds/%s",(IPTR)MutableFilePart(build_path),(IPTR)MutableFilePart(dest));
			else
				ret=0;
		}
...
short convert_theme(DOpusCallbackInfo *info,char *source,char *dest)
{
	...
	// Get source path
	strcpy(buf,source);
	if ((ptr=MutableFilePart(buf))) *ptr=0;
	if ((lock=Lock(buf,ACCESS_READ)))
	{
		old=CurrentDir(lock);
		source=MutableFilePart(source);
	}
...
void convert_theme_file(char *file)
{
	...
	if ((ptr=MutableFilePart(buf)))
	{
		...
		{
			*(MutablePathPart(buf))=0;
			strcpy(buf+256,MutableFilePart(buf));
			AddPart(buf+256,MutableFilePart(file),256);
