...
// Can't lock object
	...
	{
	strcpy( data->object_name, MutableFilePart( data->prog_name ) );
	strcpy( data->datebuf, "???" );
	data->timebuf[0] = 0;
	}
...
if	((lock = Lock( name, ACCESS_READ )))
	...
	if	(data->wb_info)
		...
		// Show info
		WBInfo( parent, MutableFilePart( data->name ), data->screen );
						...
						if	(WINREQUESTER(data->window))
							...
							*MutablePathPart( data->buffer ) = 0;
							...
							if	(!(AslRequestTags(
								...
								ASLFR_InitialFile,MutableFilePart((char*)GetGadgetValue(data->list,GAD_ICON_DEFAULT_TOOL)),
...
if	(NewIconBase)
	...
	if	(dest)
		...
		if (err)
		{
			...
			lsprintf(name,"%s.info",(IPTR)MutableFilePart(save_name));
	...
	// Save icon
	#ifdef __amigaos4__
	if (PutIconTags( save_name, icon_pointer, TAG_DONE ))
#else
...
if	(data->icon)
	...
	#ifdef __amigaos4__
	if	(PutIconTags(save_name,data->icon,TAG_DONE))
#else
