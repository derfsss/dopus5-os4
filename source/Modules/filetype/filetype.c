	...
	if	((olddirlock = CurrentDir( pathlock )))
		...
		if	((filelock = Lock( file, ACCESS_READ )))
			...
			if	(flags & EXISTF_FILE)
				{
				#ifdef __amigaos4__
				if	(ExamineObject_to_FIB( filelock, fib ) && fib->fib_DirEntryType < 0)
				#else
...
name = MutableFilePart( path );
*name = 0;
name = MutableFilePart( data->current_entry_path );
	...
	if	(ok)
		...
		if	((lock = Lock( "DOpus5:Storage/Filetypes", ACCESS_READ )))
			...
			oldlock = CurrentDir( lock );
				...
				if ((context = ObtainDirContextTags(EX_LockInput, lock, EX_DataFields, EXF_ALL, TAG_END)))
					{
					while ((exdata = ExamineDir(context)))
						{
						if (!EXD_IS_DIRECTORY(exdata))
							{
							if	((ftl = ReadFiletypes( exdata->Name, data->filetype_cache->memory )))
								{
								if	(!Att_NewNode( data->filetype_cache, NULL, (ULONG)ftl, 0 ))
									{
									ok = FALSE;
		...
		while	((entry = (FunctionEntry *)REFCALL(func_callback, EXTCMD_GET_ENTRY, IPCDATA(ipc), 0)))
			...
			SetGadgetValue( data->list, GAD_FIND_TEXT1,
				(ULONG)(strlen( data->current_entry_path ) > FILENAME_MAXLEN
					? (char *)MutableFilePart( data->current_entry_path )
